...
function displayWeather(data){
  ...
  weatherDetails.innerHTML=`
   <span><img src="source/icons/wind.svg" alt="wind"><span>  ${windSpeed} KM/h</span></span>
    <span><img src="source/icons/droplets.svg" alt="humidity"> <span> ${humidity}</span></span>`;
...
async function main() {
  ...
  search.addEventListener("click", async() => {
    ...
    if (value.trim() === "") {
      ...
      inp.value="";
      const data =  getWeather(value).then(v=>{
        displayWeather(v);
      }).catch(e=>{
        alert(`${e} this city name might be wrong or never exists !!`)
      })
